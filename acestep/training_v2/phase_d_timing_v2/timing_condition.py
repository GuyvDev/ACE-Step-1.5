...
from acestep.training_v2.phase_d_timing_v2.length_regulator import length_regulate
...
def _event_feature(
    ...
    note_group_duration = max(float(event.get("note_group_duration_sec", duration)), 1e-6)
    ...
    if config.explicit_duration:
        values.extend([
            ...
            min(2.0, duration / note_group_duration) - 1.0,
        ])
    return values


def build_timing_condition(
    events: list[dict[str, Any]],
    target_frames: int,
    config: PhaseD2Config,
) -> tuple[torch.Tensor, torch.Tensor, dict[str, Any]]:
    """Expand primary timing events and record the per-frame active-control mask."""
    if not events:
    ...
    event_active = torch.tensor([
        bool(event.get("control_active", event.get("target_phrase_mask", False)))
        for event in events
    ])
    control_active_mask = event_active[event_ids]
    if config.melody_prosody:
    ...
    audit.update({
        "schema": "phase_d_timing_v2_condition_v1",
        ...
        "control_active_mask": control_active_mask.tolist(),
        "phoneme_role": "auxiliary_identity_only",
    })
