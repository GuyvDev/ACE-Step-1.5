...
def test_timing_condition_has_exact_frame_budget_and_duration_fields():
    """Expansion is exact and exposes the explicit-duration group."""
    config = PhaseD2Config()
    dense, event_ids, audit = build_timing_condition(_events(), 15, config)
...
def test_timing_condition_marks_silence_and_event_boundaries():
    """Silence and note boundary channels follow the allocated events."""
    config = PhaseD2Config()
    dense, event_ids, _ = build_timing_condition(_events(), 15, config)
...
def test_timing_condition_rejects_empty_or_nonpositive_events():
    """Invalid event lists fail closed before reaching the real model."""
    config = PhaseD2Config()
    with pytest.raises(ValueError, match="at least one event"):
        build_timing_condition([], 10, config)
    invalid = _events()
    invalid[0]["duration_sec"] = 0.0
    with pytest.raises(ValueError, match="finite and positive"):
        build_timing_condition(invalid, 10, config)
...
def test_absolute_timing_relocates_for_counterfactual_phrase_shifts():
    ...
    for shift in (0.0, -0.5, 0.5):
        dense, event_ids, audit = build_timing_condition(_shifted_events(shift), 40, config)
