...
class PhaseD2Config:
    ...
    Attributes:
        ...
        schema_version: Sidecar format version; must be "phase_d_timing_v2_sidecar_v1".
    ...
    injection_layer_indices: List[int] | None = field(default_factory=lambda: [0, 4, 8, 12])

    # Condition composition
    condition_dim: int = 0  # Computed by compute_condition_dim()
    zero_init: bool = True
    control_schedule: str = "constant"
    schema_version: str = "phase_d_timing_v2_sidecar_v1"
    ...
    def __post_init__(self) -> None:
        ...
        if self.schema_version != "phase_d_timing_v2_sidecar_v1":
            raise ValueError(
                f"Unsupported schema_version: {self.schema_version}. "
                "Must be 'phase_d_timing_v2_sidecar_v1'."
            )
    ...
    def compute_condition_dim(self) -> None:
        ...
        Group dimensions (hardcoded per schema v1):
            ...
            explicit_duration: 4 (phrase, word, event, event/note-group ratio)
        """
        dim = 0
        if self.linguistic:
            dim += 4
