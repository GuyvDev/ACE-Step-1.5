...
class InfeasibleEdit(RuntimeError):
    """Raised when a local timing edit would require global resampling."""
...
def _allocation(
    ...
    physical_total = max(
        values.numel(),
        seconds_to_frames(float(values.sum()), frame_rate_hz),
    )
    ...
    if abs(audit["global_resampling_factor"] - 1.0) >= 1e-9:
        raise InfeasibleEdit(
            "local timing edit requires forbidden global resampling: "
            f"factor={audit['global_resampling_factor']:.12f}"
        )
    return counts, audit
